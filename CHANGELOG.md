...
- Default date range (`MyWindow--createCalendarTab.cpp`): start and end dates now default to the first and last days of the next calendar month, so the most common use case (generating next month's pages) requires no manual date adjustment at startup.
